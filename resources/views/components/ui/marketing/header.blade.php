    ...
    <div class="relative z-20 flex items-center justify-between w-full h-20 max-w-6xl px-6 mx-auto">
        <div x-data="{ mobileMenuOpen: false }" class="relative flex items-center md:space-x-2 text-neutral-800">
            ...
            <div class="relative z-50 flex items-center w-auto h-full">
                <a wire:navigate href="{{ route('home') }}" class="flex items-center mr-0 md:mr-5 shrink-0">
                    <x-ui.logo class="block w-auto text-gray-800 fill-current h-9 dark:text-gray-200" />
                </a>
            ...
            <div :class="{ 'flex' : mobileMenuOpen, 'hidden md:flex' :  !mobileMenuOpen }" class="fixed pl-2 top-0 left-0 z-40 flex-col items-start justify-start hidden w-full h-full min-h-screen pt-20 space-y-5 text-sm font-medium duration-150 ease-out transform md:pt-0 text-neutral-500 md:h-auto md:min-h-0 md:left-auto md:items-center md:relative">
                <nav class="flex flex-col w-full p-6 space-y-2 bg-white md:p-0 md:flex-row md:space-x-2 md:space-y-0 md:w-auto md:bg-transparent md:flex">
                    <x-ui.nav-link wire:navigate href="/genesis/about">About</x-ui.nav-link>
                </nav>
            </div>
        </div>
        ...
        @auth
                <div class="flex items-center w-auto">
                    <x-ui.button wire:navigate type="primary" submit="true" tag="a" href="{{ route('dashboard') }}">View Dashboard</x-ui.button>
                </div>
            @else
                <div class="flex items-center w-auto">
                    <x-ui.button wire:navigate type="secondary" submit="true" tag="a" href="{{ route('login') }}">Login</x-ui.button>
                </div>
                <div class="flex items-center w-auto">
                    <x-ui.button wire:navigate type="primary" submit="true" tag="a" href="{{ route('register') }}">Sign Up</x-ui.button>
                </div>
            @endauth
